...
preferences
{
	section ("Auto-Lock...")
    	{
		input "contact0", "capability.contactSensor", title: "Which contact sensor?", required: true
        	input "lock0","capability.lock", title: "Which lock?", required: true
        	input "autolock_delay", "number", title: "Delay for auto-Lock after door is closed? (Seconds)", required: true, defaultValue: 0
        	input "relock_delay", "number", title: "Delay for re-lock w/o opening door? (Seconds)", required: true, defaultValue: 300
        	input "locking_attempts", number, title: "How many times should we send the locking command?", required: true, defaultValue: 1
        	input "locking_attempt_delay", number, title: "Delay between attempts?", required: true, defaultValue: 1
        	input "leftopen_delay", "number", title: "Notify if door open for X seconds.", required: true, defaultValue: 300
        	input "push_enabled", "enum", title: "Enable NORMAL push notifications?", metadata: [values: ["Yes","No"]], required: true, defaultValue: Yes
            	input "debug_notify", "enum", title: "Enable DEBUG push notifications?", metadata: [values: ["Yes","No"]], required: true, defaultValue: No
    	} 
}
